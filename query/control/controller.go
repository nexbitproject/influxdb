...
func (c *Controller) compileQuery(q *Query, compiler flux.Compiler) (err error) {
	logger := c.logger.With(influxlogger.Trace(q.parentCtx)...)
	...
	defer func() {
		if e := recover(); e != nil {
			...
			if entry := logger.Check(zapcore.InfoLevel, "panic during compile"); entry != nil {
...
func (c *Controller) executeQuery(q *Query) {
	...
	defer func() {
		if e := recover(); e != nil {
			...
			q.setErr(err)
			if entry := c.logger.With(influxlogger.Trace(q.parentCtx)...).
